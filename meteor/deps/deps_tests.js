...
it("deps - nested run", function(done) {
...
it("deps - lifecycle", function(done) {
    ...
    test.deepEqual(buf, [4]);
    c1.invalidate();
    test.equal(runCount, 1);
    test.equal(c1.invalidated, true);
    test.equal(c1.stopped, false);
    test.deepEqual(buf, [4, 1, 3]);

    Deps.flush();

    test.equal(runCount, 2);
    test.equal(c1.invalidated, false);
    test.deepEqual(buf, [4, 1, 3, 9, 2, 5, 7, 10]);

    // test self-stop
    buf.length = 0;
    shouldStop = true;
    c1.invalidate();
    test.deepEqual(buf, [6, 8]);
    Deps.flush();
    test.deepEqual(buf, [6, 8, 14, 11, 13, 12, 15]);
    done()
});

it("deps - onInvalidate", function(done) {
    var buf = "";
...
it('deps - invalidate at flush time', function(done) {
...
it('deps - throwFirstError', function(done) {
    var d = new Deps.Dependency;
