it("logging - _getCallerDetails", function(done) {
    ...
    // stacktrace.
    if ((new Error).stack) {
        test.equal(details.file, 'runnable.js');

        // evaled statements shouldn't crash
        var code = "Log._getCallerDetails().file";
        test.match(eval(code), /^eval|runnable.js$/);
    }
    done()
});
...
it("logging - parse", function(done) {
...
it("logging - format", function(done) {
