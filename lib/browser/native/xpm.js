...
extend(xpm, {
    ...
    config: function(opts) {
        if (opts.src) this._srcpath = opts.src
        if (opts.map) extend(this._sourceMap, opts.map)
    },
    define: function(packageName, depArr, fn) {
        ...
        depArr.forEach(function(dep) {
            if (!cache[dep]) {
                self._loadSrc(dep + "/"+dep.replace("/","-")+".js")
            }
    ...
    use: function(names, cb, doNotExec) {
        var self = this
        var cbArgs = []
        ...
        names.forEach(function(name, index) {
            ...
            if (!self._cache[packname]) {
                self._loadSrc(packname + "/"+packname.replace("/","-")+".js")
            }
    ...
    _execFile: function(pathDetail) {
        ...
        //read file cache
        if (p._fileCache[curpath]) return p._fileCache[curpath].exports
        //set require
        ...
        //can be recurse
        var _module = p._fileCache[curpath] = {exports:{}}
        if (p._allfiles[curpath]) {
            try {
                p._allfiles[curpath].call(null, _require, _module.exports, _module)
            } catch(e) {
            ...
            if (!pathDetail.ismain) {
                ...
                throw new Error(p._msg("not add main file: " + curpath))
            }
        }
    ...
    _getRequirePathDetail: function(pathStr, curpath, p) {
        ...
        if (!p && pathStr.charAt(0) === ".") {
            ...
            if (relativepath.charAt(0) === ".")
                _throw ("out of current package workspace.")
            ismain = relativepath === ""
            packname = p._name
        } else {
    ...
    _loadSrc: function(src) {
        var self = this
        if (self._sourceMap[src])
            src = self._sourceMap[src]
        if (self._readyLoad[src]) return
        loadSrc(self._srcpath + "/" + src, function() {
...
extend(Package.prototype, {
    ...
    addStyle: function(filepath, styleContent) {
        ...
        var styleID = this._name + "/" + filepath
        var el = document.createElement('style')
        el.innerHTML = styleContent
        el.setAttribute("id", styleID)
        head.appendChild(el)
        this._fileCache[filepath] = {exports:styleID}
    },
    addFile: function(filepath, fn, canNotBePreload) {
        this._allfiles[filepath] = fn
        if (canNotBePreload)
            this._noPreloadFiles.push(filepath)
    },
    ...
    preloadAll: function() {
        ...
        self._endedArr.push(function() {
            for (var filename in self._allfiles) {
                if (~self._noPreloadFiles.indexOf(filename)) continue
