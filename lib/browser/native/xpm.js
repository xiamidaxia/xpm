    ...
    define: function(packageName, depArr, fn) {
        ...
        var checkLoaded = function(loadedDep) {
            ...
            if (_depLoadingCount === 0) {
                ...
                if (_p._tobeTest) {
                    if (_p._testFiles.length !== 0) {
                        _p._testFiles.forEach(function(testFile) {
                            xpm._execFile({packname: packageName, ismain:false, path:testFile})
                        })
                    } else {
                        console.warn("[client] no test files in '" +packageName +"'")
                    }
                }
                _p.loaded = true
...
extend(Package.prototype, {
    addStyle: function(filepath, styleContent) {
        ...
        head.appendChild(el)
        this._allfiles[filepath] = styleID
    },
