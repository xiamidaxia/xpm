...
_.extend(XpmClient.prototype, {
    ...
    run: function(cb) {
        ...
        self.on('added', function(pack) {
            ...
            console.log('added package: ' + pack)
    ...
    _createXpmFile: function() {
        ...
        if (this._testPackages.length !== 0) {
            ...
            content = content
                + _start
                + 'xpm.use(["xpm/chai","xpm/mocha","xpm/sinon","xpm/sinon-chai"], function(chai, mocha) {\n'
                + '    chai("' + testType + '")\n'
                + '    mocha.setup(' + JSON.stringify(mochaOpts) + ')\n'
                + '    xpm.use(' + JSON.stringify(this._testPackages) + ', function(){mocha.run()}, true)\n'
                + '})'
                + _end
        }
        var buf = new Buffer(_start + content + _end)
        this._xpmFileBuf = buf
    ...
    addPackage: function(family, packageName) {
        ...
        //p._data.imports.forEach()
        this._renderPackage(p, fullname)
        return p
    },
    /**
     * @param {Package} p
     ...
     * @private
     */
    _renderPackage: function(p, fullname) {
        var allfiles
        var streams = []
        var self = this
    ...
    _createManifestAndMap: function() {
        ...
        manifestBuf.push(self._config.static_url + "/xpm.js")
        manifestBuf.push(self._config.static_url + "/xpm_map.js")
        _.each(this._manifestMap, function(source) {
            manifestBuf.push(self._config.static_url + "/" + source)
        })
        manifestBuf.push("NETWORK:")
        manifestBuf.push("*")
