    ...
    _createXpmFile: function() {
        ...
        defaultNatives.forEach(function(native) {
            content = content
                ...
                + fs.readFileSync(path.join(__dirname, "browser/native/" + native + ".js")).toString()
                + _wrapEnd
        })
        ...
        _.each(self._familyMap, function(familypath, familyname) {
            watcher(familypath, function(filepath) {
                try {
                    ...
                    if (_p.length >= 2) {
                        //change package
                        if (fs.existsSync(path.join(familypath, _p[0], "package.js"))) {
                            self.updatePackage(familyname, _p[0])
                        }
                    } else if (_p.length === 1 && _p[0] === "package.js") {
                        //update all packages
                        _.each(self._map, function(pack, key) {
                            key = key.split("/")
                            if (key[0] === familyname  && fs.existsSync(path.join(familypath, key[1], "package.js"))) {
                                self.updatePackage(familyname, key[1])
                            }
                        })
                    }
                } catch (e) {
