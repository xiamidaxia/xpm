...
_.extend(XpmServer.prototype, {
    ...
    addPackage: function(family, packageName) {
        ...
        if (p._data.all_preload) {
            //exec all loaded files when creating package
            self._execAllFiles(p)
        } else if (p._data.main_preload) {
            //exec main file when creating package
            self.require(fullname)
        }
        return p
    },
    _execAllFiles: function(p) {
        var self = this
        p.getFiles().forEach(function(filepath) {
            var extname = path.extname(filepath)
            var realpath = path.join(p._path, filepath)
            if (!fs.statSync(realpath).isDirectory() && (extname === ".js" || extname === ".coffee")) {
                self.require(p._family + "/" + p._name + "/" + filepath)
            }
    ...
    _execFile: function(pathDetail) {
        ...
        _context.require = function(pathStr, isNative) {
            var _detail, fullname, curfullname
