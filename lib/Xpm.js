...
function Xpm(config) {
    ...
    this._map = {}
    this._addDefaultFamily()
    if (config.family) {
        this.addFamily(config.family)
    }
}
...
_.extend(Xpm.prototype, {
    ...
    _addDefaultFamily: function() {
        this.addFamily({
            xpm: path.join(__dirname,"..","xpm")
        })
    },
    /**
     * @param family
    ...
    /**
     * @param packageArr
     *      eg ["meteor/*", "other/pack1"]
     * @return {Array} ["meteor/deps", "other/pack1"...]
     */
    getFullPackages: function(packageArr) {
        ...
        packageArr.forEach(function(item) {
            ...
            var familyPath = self.getFamilyPath(item[0])
            if (item[1] === "*") {
                fs.readdirSync(familyPath).forEach(function(packname) {
                    var realpath = path.join(familyPath, packname)
                    if (fs.statSync(realpath).isDirectory()) {
                        _allTestPackages.push(item[0] + "/" + packname)
                    }
                })
            } else {
                _allTestPackages.push(item.join("/"))
            }
        })
        return _allTestPackages
    }
})
